...
def _check_response(response: httpx.Response, authentication=False) -> httpx.Response:
    ...
    if not authentication and header_cookies and "i=0; secure" in header_cookies:
...
class Request:
    ...
    def post(self, *args: tuple, authentication=False, **kwargs: dict[str, any]) -> httpx.Response:
        """Return a response using the post function from httpx.Client."""
        response = self.client.post(*args, **kwargs)

        return _check_response(response, authentication)

    def get(self, *args: tuple, authentication=False, **kwargs: dict[str, any]) -> httpx.Response:
        """Return a response using the get function from httpx.Client."""
        response: httpx.Response = self.client.post(*args, **kwargs)

        return _check_response(response, authentication)

    def head(self, *args: tuple, authentication=False, **kwargs: dict[str, any]) -> httpx.Response:
        """Return a response using the head function from httpx.Client."""
        response: httpx.Response = self.client.head(*args, **kwargs)

        return _check_response(response, authentication)

    def request(self, *args: tuple, authentication=False, **kwargs: dict[str, any]) -> httpx.Response:
